...
def leader_election(client, server_name):
    ...
    # expire as well.
    # Here, if the client fails to renew the lease for 50 seconds (network
    # partition or maching goes down), then the leader election key will 
    # expire.
    # https://help.compose.com/docs/etcd-using-etcd3-features#section-leases
    lease = client.lease(5)

    # Try to create they key with your name as the value. If it fails, then
...
def on_leadership_gained(lease):
    while True:
        ...
        try:
            ...
            print("\nRevoking the lease. No longer the leader.")
...
def wait_for_next_election(client):
    ...
    def watch_callback(resp):
        for event in resp.events:
            ...
            # it means the key expired / got delted, which means the
...
# Try to insert a key into etcd with a value and lease. If the lease expires
...
def try_insert(client, key, value, lease):
    insert_succeed, _ = client.transaction(
        failure=[],
